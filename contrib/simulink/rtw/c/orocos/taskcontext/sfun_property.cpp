...
#define MDL_CHECK_PARAMETERS
  ...
  static void mdlCheckParameters(SimStruct *S)
  {
      ...
      /* Check 2nd parameter: value */
      {
          if (!mxIsDouble(OUT_PARAM(S)) ||
...
namespace {
    struct PropData {
        PropData(Property<double>* pprop, bool cleanup )
...
#define MDL_START  /* Change to #undef to remove function */
  ...
  static void mdlStart(SimStruct *S)
  {
    ...
    if ( RTW::currentTC ) {
        ...
        // value
        double value = mxGetScalar( OUT_PARAM(S) );
        bool own_prop (false);
        RTT::Property<double>* prop = 0;
        if ( RTW::currentTC->properties()->find( name ) == 0 ) {
            prop = new RTT::Property<double>( name, desc, value );
            ...
            prop = RTW::currentTC->properties()->getProperty<double>( name );
            if (!prop) {
                log(Error) <<"Property< double > "<< name << " has wrong type in TaskContext." <<endlog();
