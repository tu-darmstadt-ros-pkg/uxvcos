...
#define S_FUNCTION_NAME  sfun_ioutputport
...
#ifndef MATLAB_MEX_FILE
namespace {
    typedef RTT::OutputPort< int > IntOutputPort;

    struct IntPort
    {
        IntPort(std::string name, int init )
            : port( new IntOutputPort(name) ), d(init), cleanup(false) {} // See bug #579: Simulink component configuration procedure broken when using DeploymentComponent
        IntPort( IntOutputPort* dp, int init )
  ...
  static void mdlStart(SimStruct *S)
  {
    ...
    if ( RTW::currentTC ) {
        ...
        if ( RTW::currentTC->ports()->getPort( name ) == 0 ) {
            ...
            dport = new IntPort( RTW::currentTC->ports()->getPortType< OutputPort<int> >(name), (int32_T)mxGetScalar( DEFVALUE_PARAM(S) )); 
            if ( !dport->port ) {
                log(Error) <<"OutputPort< int > "<< name << " has wrong type in TaskContext." <<endlog();
        ...
        log(Info) << "Setting port " << name << " to " << (int32_T)mxGetScalar( DEFVALUE_PARAM(S) ) << endlog();
        dport->port->write( (int32_T)mxGetScalar( DEFVALUE_PARAM(S) ) );
...
#undef MDL_UPDATE  /* Change to #undef to remove function */
  ...
  static void mdlUpdate(SimStruct *S, int_T tid)
  {
      ...
      InputInt32PtrsType uPtrs  = (InputInt32PtrsType)ssGetInputPortSignalPtrs(S,0);
