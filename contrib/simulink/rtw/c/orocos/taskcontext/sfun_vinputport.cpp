...
#define S_FUNCTION_NAME  sfun_vinputport
...
#ifndef MATLAB_MEX_FILE
namespace {
    typedef RTT::InputPort< std::vector<double> > VectorInputPort;

    struct VectorPort
    {
        VectorPort(std::string name, std::vector<double> init )
            : port( new VectorInputPort(name)), vect(init), cleanup(false) {} // See bug #579: Simulink component configuration procedure broken when using DeploymentComponent
        VectorPort(VectorInputPort* dp, std::vector<double> init )
            : port( dp ), vect(init), cleanup(false) {}
        ~VectorPort() {
            if (cleanup)
                delete port;
        }
        VectorInputPort* port;
  ...
  static void mdlStart(SimStruct *S)
  {
    ...
    if ( RTW::currentTC ) {
        ...
        if ( RTW::currentTC->ports()->getPort( name ) == 0 ) {
            ...
            dport = new VectorPort( RTW::currentTC->ports()->getPortType< VectorInputPort >(name), vect ); 
            if ( !dport->port ) {
                log(Error) <<"InputPort< std::vector<double> > "<< name << " has wrong type in TaskContext." <<endlog();
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    UNUSED_ARG(tid); /* not used in single tasking mode */
      ...
      vport->port->readNewest( vport->vect );
#endif
      for (i = 0; i < ny; i++) {
          // If in Orocos, use the dataport itself, if in simulink, use the parameter.
#ifndef MATLAB_MEX_FILE
          rwork[i] = vport->vect[i];
#else
          rwork[i] = i < np ? mxGetPr(DEFVALUE_PARAM(S))[i] : 0.0;
#endif
      }
