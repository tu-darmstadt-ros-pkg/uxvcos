...
#define S_FUNCTION_NAME  sfun_iproperty
...
#define MDL_CHECK_PARAMETERS
  ...
  static void mdlCheckParameters(SimStruct *S)
  {
      ...
      // from sfun_dtype_param.c
    if(EDIT_OK(S, OUT_PARAM(S))) {
        boolean_T isOk = (mxIsNumeric(OUT_PARAM(S))  &&  
                          !mxIsLogical(OUT_PARAM(S)) &&
                          !mxIsComplex(OUT_PARAM(S)) &&  
                          !mxIsSparse(OUT_PARAM(S))  && 
                          !mxIsEmpty(OUT_PARAM(S))   &&
                          mxGetNumberOfElements(OUT_PARAM(S)) == 1);
...
//              */
//             DTypeId  dtype = ssGetDTypeIdFromMxArray(OUT_PARAM(S));
//             isOk = (dtype == SS_INT8  || 
//                     dtype == SS_INT16 || 
//                     dtype == SS_INT32);
//         }
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    ssSetOutputPortWidth(S, 0, 1); // 1 d property.
...
namespace {
    struct PropData {
        PropData(Property<int>* pprop, bool cleanup )
...
#define MDL_START  /* Change to #undef to remove function */
  ...
  static void mdlStart(SimStruct *S)
  {
    ...
    if ( RTW::currentTC ) {
        ...
        // value
        int value = (int32_T)mxGetScalar( OUT_PARAM(S) );
        bool own_prop (false);
        RTT::Property<int>* prop = 0;
        if ( RTW::currentTC->properties()->find( name ) == 0 ) {
            prop = new RTT::Property<int>( name, desc, value );
            ...
            prop = RTW::currentTC->properties()->getProperty<int>( name );
            if (!prop) {
                log(Error) <<"Property< int > "<< name << " has wrong type in TaskContext." <<endlog();
                ssSetErrorStatus(S,"Property has wrong type in current TaskContext.");
                return;
            }
        }
        ...
        ssSetErrorStatus(S,"Property: Could not locate current TaskContext.");
#endif
  }
#endif /*  MDL_START */
...
#ifndef MATLAB_MEX_FILE
     y[0] = static_cast<PropData*>(ssGetUserData(S))->prop->get();
#else
     // Only the 'value' is a runtime parameter:
     //y[0] = *((real_T *)((ssGetRunTimeParamInfo(S,0))->data));
     y[0] = (int32_T)mxGetScalar( OUT_PARAM(S) );
#endif
}
