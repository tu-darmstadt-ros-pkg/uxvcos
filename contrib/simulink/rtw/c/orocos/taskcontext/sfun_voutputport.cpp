...
#define S_FUNCTION_NAME  sfun_voutputport
...
#ifndef MATLAB_MEX_FILE
namespace {
    typedef RTT::OutputPort< std::vector<double> > VectorOutputPort;

    struct VectorPort
    {
        VectorPort(std::string name, std::vector<double> init )
            : port( new VectorOutputPort(name)), vect(init), cleanup(false) {} // See bug #579: Simulink component configuration procedure broken when using DeploymentComponent
        VectorPort(VectorOutputPort* dp, std::vector<double> init )
            : port( dp ), vect(init), cleanup(false) {}
        ~VectorPort() {
            if (cleanup)
                delete port;
        }
        VectorOutputPort* port;
...
#define MDL_START  /* Change to #undef to remove function */
  ...
  static void mdlStart(SimStruct *S)
  {
    ...
    if (!ssGetInputPortConnected(S,0)) {
        ...
        log(Warning) << ": input width was "<<ssGetInputPortWidth(S, 0) <<endlog();
#endif
    ...
    if ( RTW::currentTC ) {
        ...
        if (nu < np) {
            log(Warning) << name << ": setting inputportwidth to paramwidth " << np <<endlog();
            nu = np;
            ssSetInputPortWidth(S, 0, nu);
        }
            
        std::vector<double> vect( nu, 0.0 );
        for (int i=0; i != nu && i != np; ++i)
        ...
        if ( RTW::currentTC->ports()->getPort( name ) == 0 ) {
            ...
            dport = new VectorPort( RTW::currentTC->ports()->getPortType< VectorOutputPort >(name), vect ); 
            if ( !dport->port ) {
                log(Error) <<"OutputPort< std::vector<double> > "<< name << " has wrong type in TaskContext." <<endlog();
