...
#define S_FUNCTION_NAME  sfun_taskcontext
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    if (ssGetNumSFcnParams(S) != ssGetSFcnParamsCount(S)) {
        ...
        ssSetErrorStatus(S,"TaskContext: Number of expected != number of actual parameters.");
        return;
    }
    ...
    if ( RTW::currentTC ) {
        ssSetUserData(S, (void*) RTW::currentTC );
        log(Info) << "Succesfully located TaskContext "<< RTW::currentTC->getName() << endlog();
    }
    else
        ssSetErrorStatus(S,"TaskContext: Could not locate current TaskContext.");
#endif
}
...
#define MDL_RTW  /* Change to #undef to remove function */
  ...
  {
      /* Not neccessary, since this is only used in MEX mode.... */
      if ( !ssWriteRTWWorkVect(S, "PWork", 1,
                               "TCPointer", 1 )) {
          ssSetErrorStatus(S,"TaskContext: Error setting TCPointer work vector.");
          return;
      }
  }
#endif /* MDL_RTW */
