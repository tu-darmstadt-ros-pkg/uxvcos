...
#define S_FUNCTION_NAME  sfun_dataport
...
#ifndef MATLAB_MEX_FILE
namespace {
    typedef RTT::DataPort< double > DoubleDataPort;

    struct DoublePort
    {
        DoublePort(std::string name, double init )
            : port( new DoubleDataPort(name, init) ), d(init), cleanup(false) {} // See bug #579: Simulink component configuration procedure broken when using DeploymentComponent
        DoublePort( DoubleDataPort* dp, double init )
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    ssSetInputPortDirectFeedThrough(S, 0, 1); // Used in mdlOutputs
  ...
  static void mdlStart(SimStruct *S)
  {
    ...
    if ( RTW::currentTC ) {
        ...
        if ( RTW::currentTC->ports()->getPort( name ) == 0 ) {
            ...
            dport = new DoublePort( RTW::currentTC->ports()->getPortType< DataPort<double> >(name), mxGetScalar( DEFVALUE_PARAM(S) )); 
            if ( !dport->port ) {
                log(Error) <<"DataPort< double > "<< name << " has wrong type in TaskContext." <<endlog();
        ...
        dport->port->Set( mxGetScalar( DEFVALUE_PARAM(S) ) );
        ssSetUserData(S, (void*) dport );

        if ( ssGetInputPortConnected(S,0) ) {
            log(Info) << "DataPort " << name << " is an OUTPUT port." <<endlog();
        } else {
            log(Info) << "DataPort " << name << " is an INPUT port." <<endlog();
        }
    }
    else
