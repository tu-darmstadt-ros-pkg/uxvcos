...
#define S_FUNCTION_NAME  sfun_inputport
...
#ifndef MATLAB_MEX_FILE
namespace {
    typedef RTT::InputPort< double > DoubleInputPort;

    struct DoublePort
    {
        DoublePort(std::string name, double init )
            : port( new DoubleInputPort(name) ), d(init), cleanup(false) {} // See bug #579: Simulink component configuration procedure broken when using DeploymentComponent
        DoublePort( DoubleInputPort* dp, double init )
  ...
  static void mdlStart(SimStruct *S)
  {
    ...
    if ( RTW::currentTC ) {
        ...
        if ( RTW::currentTC->ports()->getPort( name ) == 0 ) {
            ...
            dport = new DoublePort( RTW::currentTC->ports()->getPortType< InputPort<double> >(name), mxGetScalar( DEFVALUE_PARAM(S) )); 
            if ( !dport->port ) {
                log(Error) <<"InputPort< double > "<< name << " has wrong type in TaskContext." <<endlog();
