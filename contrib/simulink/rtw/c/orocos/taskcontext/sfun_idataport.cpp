...
#define S_FUNCTION_NAME  sfun_idataport
...
#ifndef MATLAB_MEX_FILE
namespace {
    typedef RTT::DataPort< int > IntDataPort;

    struct IntPort
    {
        IntPort(std::string name, int init )
            : port( new IntDataPort(name, init) ), d(init), cleanup(false) {} // See bug #579: Simulink component configuration procedure broken when using DeploymentComponent
        IntPort( IntDataPort* dp, int init )
  ...
  static void mdlStart(SimStruct *S)
  {
    ...
    if ( RTW::currentTC ) {
        ...
        if ( RTW::currentTC->ports()->getPort( name ) == 0 ) {
            ...
            dport = new IntPort( RTW::currentTC->ports()->getPortType< DataPort<int> >(name), (int32_T)mxGetScalar( DEFVALUE_PARAM(S) )); 
            if ( !dport->port ) {
                log(Error) <<"DataPort< int > "<< name << " has wrong type in TaskContext." <<endlog();
        ...
        dport->port->Set( (int32_T)mxGetScalar( DEFVALUE_PARAM(S) ) );
